jQuery(document).ready(function($) {
    ...
    const
            ...
            htwptLoadedText = WPTEMPLATA.message.allload,
            htwptNothingFoundText = WPTEMPLATA.message.notfound,
        ...
        let
            // Project Data
            htwptProjectData = WPTEMPLATA.alldata,
                ...
                html += `<div class="${singleItem} col-xl-4 col-md-6 col-12">
                            <div class="htwpt-project-item ${singleItem}" data-group="${shareId}">
                                ...
                                <div class="htwpt-project-info">
                                    ...
                                    <h6 class="sub-title">${totalItem} ${htwpUcfirst(htwptDemosValue)} ${WPTEMPLATA.message.packagedesc}</h6>
                                </div>
                            </div>
                        </div>`;
            }
        ...
        htwptProjectGrid.on('click', '.htwpt-project-item', function (e) {
            ...
            // Projects Data Filter for Free/Pro
            if (htwptTypeValue != "all") {
                htwptProjectGroupData = htwptProjectGroupData.filter(i => i.isPro == htwptTypeValue)
            }
            // Projects Data Filter by Search
            if (htwptSearchFieldValue != "") {
            ...
            for (let i = 0; i < htwptPrintGroupData.length; i++) {
                ...
                if(isPro == '1' ){
                    $impbutton = `<a href="${WPTEMPLATA.prolink}" target="_blank">${WPTEMPLATA.buttontxt.buynow}</a>`;
                }else{
                    $impbutton = `<a href="#" class="htwpttemplateimp button" data-templpateopt='{"parentid":"${shareId}","templpateid":"${id}","templpattitle":"${title}","message":"Successfully ${htwpUcfirst(shareId)+ ' -> ' + title} has been imported.","thumbnail":"${thumbnail}","freePlugins":"${freePlugins}", "proPlugins":"${proPlugins}","requiredtheme":"${requiredtheme}" }'>${WPTEMPLATA.buttontxt.import}</a>`;
                }
                            ...
                            <div class="htwpt-project-item">
                                <div class="htwpt-project-thumb">
                                    ...
                                    <div class="htwpt-actions">
                                        <a href="${demoUrl}" target="_blank">${WPTEMPLATA.buttontxt.preview}</a>
                                        ${$impbutton}
                                    </div>
                                </div>
                                <div class="htwpt-project-info">
    ...
    */
    $('body').on('click', 'a.htwpttemplateimp', function(e) {
        ...
        var htbtnMarkuplibrary = `<a href="#" class="wptemplataimpbtn" data-btnattr='{"templateid":"${template_opt.templpateid}","parentid":"${template_opt.parentid}","templpattitle":"${template_opt.templpattitle}"}'>${WPTEMPLATA.buttontxt.tmplibrary}</a>`;
        var htbtnMarkuppage = `<a href="#" class="wptemplataimpbtn htwptdisabled" data-btnattr='{"templateid":"${template_opt.templpateid}","parentid":"${template_opt.parentid}","templpattitle":"${template_opt.templpattitle}"}'>${WPTEMPLATA.buttontxt.tmppage}</a>`;
        ...
        $.ajax( {
            ...
            type: 'POST',
            data: {
                action: 'htwptemplata_ajax_get_required_plugin',
    ...
    $('body').on('click', 'a.wptemplataimpbtn', function(e) {
        ...
        $.ajax({
            url: WPTEMPLATA.ajaxurl,
            data: {
                'action'       : 'wptemplata_ajax_request',
            ...
            success:function(data) {
                $(".htwptmessage").show();
                var tmediturl = WPTEMPLATA.adminURL+"/post.php?post="+ data.id +"&action=elementor";
